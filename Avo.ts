// Generated by Avo VERSION 178.33.0. You should never have to make changes to this file.
...
export const avoInspectorApiKey = "ZHESFuV6PgRRj0FUtK0p";
// @ts-ignore
  ...
  if (_avo_sampling_rate > 0) {
    if (Math.random() < _avo_sampling_rate) {
      ...
      fetch("https://api.avo.app/i", {
        ...
        body: JSON.stringify({
          "ac": "RQIHim6VxA4bDZ63ChB6",
          "br": "WucaDxijGqQAuDq0MKTSj",
          "en": env,
          "ev": eventId,
          "ha": hash,
          "sc": "CjQNpZ3sI4j4zwzCN54j",
          "se": (new Date()).toISOString(),
          "so": "dKj4_N1BsVBTdKe4eKjk2",
...
export function initAvo(options: {env: AvoEnv;
    ...
    GoogleAnalytics4Destination: CustomDestination) {
  if (__AVO_ENV__ !== null) {
    return;
  }
...
(function() {
  ...
  if (!__AVO_NOOP__) {
    ...
    if (__AVO_ENV__ === 'prod') {
      GoogleAnalytics4 && GoogleAnalytics4.make && GoogleAnalytics4.make(__AVO_ENV__, null);
...
export interface BaRegistrationProperties {
  gaEventType: string;
  hotjarEventType: string;
}
/**
 * ba_registration: The user has registered for a Basic Account
 * 
 * @param properties the properties associatied with this event
 * @param properties.gaEventType: This property is sent to GA4 through MarTech's martechagents script loaded from GTM.
 * @param properties.hotjarEventType: no description
 * 
 * @see {@link https://www.avo.app/schemas/CjQNpZ3sI4j4zwzCN54j/branches/WucaDxijGqQAuDq0MKTSj/events/szlFPwp0acv-RlZ-bI_p2}
 */
export function baRegistration(properties: BaRegistrationProperties) {
  // @ts-ignore
  let eventPropertiesArray: array = [];
  eventPropertiesArray.push({id: "NtqEtjlL7OrCHdUKYQ2Yv", name: "gaEventType", value: properties.gaEventType});
  eventPropertiesArray.push({id: "1CwvE7ybQlgElb8jc9SVn", name: "hotjarEventType", value: properties.hotjarEventType});
  ...
  if (__AVO_ENV__ !== AvoEnv.Prod || __WEB_DEBUGGER__) {
    ...
    // debug console in Avo
    if (!__AVO_NOOP__) {
      _avo_invoke(__AVO_ENV__, "szlFPwp0acv-RlZ-bI_p2", "a604d642ad58d572e4d7adf8cfe96971976cf870b74bf4edff04f10df91cdebe", messages.map(m => Object.assign({}, {tag: m.tag, propertyId: m.propertyId, additionalProperties: m.additionalProperties, actualType: m.actualType})), 'event');
    }
    InternalAvoLogger.logEventSent("ba_registration", eventProperties, userProperties);
    if (__WEB_DEBUGGER__) {
      // Avo web debugger
      _avo_debugger_log("szlFPwp0acv-RlZ-bI_p2", "ba_registration", messages, eventPropertiesArray, userPropertiesArray, []);
    }
  }
  if (!__AVO_NOOP__) {
    if (__INSPECTOR__ != null) {
      // @ts-ignore
      __INSPECTOR__._avoFunctionTrackSchemaFromEvent("ba_registration", eventProperties, "szlFPwp0acv-RlZ-bI_p2", "a604d642ad58d572e4d7adf8cfe96971976cf870b74bf4edff04f10df91cdebe");
    }
    // destination GoogleAnalytics4
    GoogleAnalytics4.logEvent("ba_registration", (Object as any).assign({}, eventProperties));
  } else {
    // do nothing
  }
}
